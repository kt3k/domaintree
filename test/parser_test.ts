...
`;
  ...
  if (doc.aggregates.length !== 1) {
    throw new Error("aggregates length mismatch");
  }

  const agg = doc.aggregates[0];
  if (agg.name !== "Order") throw new Error("aggregate name mismatch");
  if (agg.description !== "Order aggregate") {
    throw new Error("aggregate description mismatch");
  }
  if (agg.root.name !== "Order") throw new Error("root name mismatch");
  if (agg.root.type !== "entity") throw new Error("root type mismatch");
  if (agg.root.properties?.length !== 1) {
    throw new Error("root properties length mismatch");
  }
  if (agg.root.properties?.[0].name !== "id") {
    throw new Error("root property name mismatch");
  }
  if (agg.root.children?.length !== 2) {
    throw new Error("root children length mismatch");
  }
  ...
  if (vo.properties?.length !== 1) {
    throw new Error("child VO properties length mismatch");
  }
  ...
  if (enumModel.values?.length !== 2) {
    throw new Error("child enum values length mismatch");
  }
  if (enumModel.values![0] !== "PENDING") {
    throw new Error("enum value[0] mismatch");
  }
});
